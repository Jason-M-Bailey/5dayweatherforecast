...
let cities = ["greensboro", "new york", "san francisco", "miami", "chicago"];
...
function getCityWeather() {
  ...
  $.ajax({
    ...
    .then((res) => {
      ...
      $("#today").empty();
      let todaysContainer = $(`
            <h3 class="mt-5 city-heading">${res.name} -  ${moment().format(
        "dddd, MMMM D, YYYY"
      )} </h3>
            <i class="today wi ${setWeatherIcon(res.weather[0].icon)}"></i>
            <ul class="temp-list">
                <li>Temperature: <span class="temp">${
                  res.main.temp
                } F</span></li>
                <li>Humidity: <span class="humidity">${
                  res.main.humidity
                }%</span></li>
                <li>Wind Speed: <span class="wind-speed">${
                  res.wind.speed
                } MPH</span></li>
